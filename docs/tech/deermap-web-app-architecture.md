# deermap-web-app 前端应用 — 详细技术架构文档

> 版本：v1.1 · 2026-04-21  
> 定位：Next.js 15 App Router 全栈前端，承担页面渲染、BFF 聚合层、Payload CMS 管理后台三重职责。  
> 本文档逐步解释 `04-deermap-web-app-init.md` 每一步的目的与意义，并从架构合理性、扩展性、可维护性角度进行审视和补充。
...
deermap-web-app 是一个「三合一」应用，运行在同一个 Next.js 进程（port 3000）中：

```
┌─────────────────────────────────────────────────────┐
│                   deermap-web-app (port 3000)               │
│                                                     │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────┐  │
│  │  前端页面     │  │  BFF 层      │  │  Payload │  │
...
> **注意：** 使用 pnpm 而非 npm/yarn，与 deermap-core-service 保持一致，节省磁盘空间（硬链接 node_modules），CI 速度更快。
...
| `@payloadcms/db-postgres` | PostgreSQL 数据库适配器，共用 deermap-web-app 的 `DATABASE_URL` |
...
行业讯息**无独立页面**，以首页子模块形式展示，`wechatArticleUrl` 字段存储公众号文章链接，点击跳转公众号（与公众号联动运营）。`isPublished` 控制草稿/上线状态，`order` 控制首页展示顺序。无需 `slug` 字段（无站内详情页）。
...
BFF（Backend for Frontend）层是 deermap-web-app 最重要的架构模式，**明确隔离了「浏览器可达的接口」与「NestJS 后端」**。
...
| Multi-repo 类型同步依赖 OpenAPI | 手动维护有滞后风险 | 后期考虑在 CI 中加入「deermap-core-service Swagger JSON → deermap-web-app types 自动生成」脚本（openapi-typescript） |
...
│   │   │   ├── bioinformatics/page.tsx    ← /bioinformatics
│   │   │   ├── consulting/page.tsx        ← /consulting（行业咨询，付费 1v1）
│   │   │   ├── user/
