...
# 创建项目
nest new deermap-core-service --package-manager pnpm --strict

cd deermap-core-service
```
...
enum OrderType {
  ...
  PAPER             // 论文业务
  BIOINFORMATICS    // 生信业务（后台创建）
  CONSULTING        // 行业咨询（后台创建）
}
...
WECHAT_API_V3_KEY=
WECHAT_PRIVATE_KEY=        # 商户 API 私钥内容（多行用 \n 转义）
WECHAT_APP_ID=             # 公众号 / 小程序 AppID（JSAPI 支付用）
WECHAT_NOTIFY_URL=https://yourdomain.com/webhook/wechatpay

# 支付宝开放平台
ALIPAY_APP_ID=
ALIPAY_PRIVATE_KEY=
ALIPAY_PUBLIC_KEY=         # 支付宝平台公钥
ALIPAY_NOTIFY_URL=https://yourdomain.com/webhook/alipay
ALIPAY_RETURN_URL=https://yourdomain.com/order/callback

# 通知
NOTIFY_EMAIL=your@email.com

# 允许跨域来源（生产为域名，本地为 localhost:3000）
