...
2. Propose specific edits to `.github/copilot-instructions.md` that would accommodate the request.
...
When the user requests implementing a new feature, adding a new module/page, or modifying/adjusting any existing feature or module, ALWAYS read `.github/skills/business-analyst/SKILL.md` first and perform the BA review BEFORE generating any implementation code. Implementation may only proceed after the analysis is delivered and acknowledged.

When working on any UI, components, pages, or styling tasks, ALWAYS read `.github/skills/frontend-design/SKILL.md` first before generating any code.

When writing or modifying any React/TSX code, ALWAYS read `.github/skills/react-best-practices/SKILL.md` first before generating any code.
...
Reserved directory for upcoming AI capabilities:

- `.github/prompts/` — For reusable prompt templates (VS Code Copilot `.prompt.md` files).
