...
function LiveChart({
  ...
  chartId: string
  palette: { nameKey: string; colors: string[]; filter: string }
  ...
  const cha = (key: string) => t(`charts.${chartKey}.${key}`, { returnObjects: true }) as string[];
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <LineChart data={data}>
          ...
          <XAxis dataKey="x" tick={xTickProps} label={{ ...xLabelProps, value: ch('xAxis') }} />
          <YAxis tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Line type="monotone" dataKey="a" name={c('genes.a')} stroke={C[0]} strokeWidth={2} dot={{ r: 3 }} />
          <Line type="monotone" dataKey="b" name={c('genes.b')} stroke={C[1]} strokeWidth={2} dot={{ r: 3 }} />
          <Line type="monotone" dataKey="c" name={c('genes.c')} stroke={C[2]} strokeWidth={2} dot={{ r: 3 }} />
          <Line type="monotone" dataKey="d" name={c('genes.d')} stroke={C[3]} strokeWidth={2} dot={{ r: 3 }} />
        </LineChart>
      </ResponsiveContainer>
    );
  }
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <BarChart data={data}>
          ...
          <YAxis tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Bar dataKey="a" name={ch('groupA')} fill={C[0]} radius={[3, 3, 0, 0]} />
          <Bar dataKey="b" name={ch('groupB')} fill={C[1]} radius={[3, 3, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    );
  }
  ...
  if (chartId === 'scatter' || chartId === 'pca' || chartId === 'volcano') {
    const makeScatter = (n: number, cx: number, cy: number, sx: number, sy: number, colorIdx: number) =>
      Array.from({ length: n }, (_, i) => ({
        ...
        group: `${c('groupPrefix')} ${String.fromCharCode(65 + colorIdx)}`,
    ...
    const xLabel = ch('xAxis');
    const yLabel = ch('yAxis');
    return (
      <ResponsiveContainer width={w} height={h}>
        <ReScatterChart>
          <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
          ...
          {[0, 1, 2].map(gi => (
            <Scatter key={gi} name={`${c('groupPrefix')} ${String.fromCharCode(65 + gi)}`}
              data={groups.filter((g: {group: string}) => g.group === `${c('groupPrefix')} ${String.fromCharCode(65 + gi)}`)}
  ...
  if (chartId === 'pie') {
    const data = [
      { name: ch('proteinCoding'), value: 65 },
      { name: ch('lncrna'), value: 15 },
      { name: ch('mirna'), value: 8 },
      { name: ch('rrna'), value: 5 },
      { name: ch('trna'), value: 4 },
      { name: ch('others'), value: 3 },
    ];
    return (
      <ResponsiveContainer width={w} height={h}>
        <PieChart>
  ...
  if (chartId === 'venn') {
    ...
    const sets = [
      { x: w * 0.35, y: h * 0.42, r, label: ch('setA'), n: 55 + Math.floor(seededRandom(0) * 20) },
      { x: w * 0.5, y: h * 0.42, r, label: ch('setB'), n: 45 + Math.floor(seededRandom(50) * 20) },
      { x: w * 0.425, y: h * 0.62, r, label: ch('setC'), n: 35 + Math.floor(seededRandom(100) * 20) },
    ];
    ...
    return (
      <svg width={w} height={h} style={{ maxWidth: '100%' }}>
        <text x={w / 2} y={25} textAnchor="middle" style={{ fontSize: titleFontSize, fontWeight: 'bold', fill: THEME_COLORS.primary }}>{ch('title')}</text>
        {sets.map((s, i) => (
          <g key={i}>
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <AreaChart data={data}>
          ...
          <YAxis tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Area type="monotone" dataKey="a" name={c('pathways.a')} stackId="1" fill={C[0]} fillOpacity={0.7} stroke={C[0]} />
          <Area type="monotone" dataKey="b" name={c('pathways.b')} stackId="1" fill={C[1]} fillOpacity={0.7} stroke={C[1]} />
          <Area type="monotone" dataKey="c" name={c('pathways.c')} stackId="1" fill={C[2]} fillOpacity={0.7} stroke={C[2]} />
        </AreaChart>
      </ResponsiveContainer>
    );
  }
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <BarChart data={data}>
          ...
          <Tooltip /><Legend {...legendProps} />
          {['a', 'b', 'c', 'd', 'e'].map((k, i) => (
            <Bar key={k} dataKey={k} name={cha('series')[i]} stackId="1" fill={C[i]} />
          ))}
        </BarChart>
      </ResponsiveContainer>
    );
  ...
  // ====== ??? ======
  if (chartId === 'radar') {
    const radarMetrics = cha('metrics');
    const data = radarMetrics.map((metric, i) => ({
      metric,
      a: [0.92, 0.88, 0.90, 0.89, 0.90, 0.94][i],
      b: [0.85, 0.92, 0.88, 0.91, 0.88, 0.90][i],
      c: [0.78, 0.85, 0.82, 0.84, 0.81, 0.86][i],
    }));
    return (
      <ResponsiveContainer width={w} height={h}>
        <RadarChart data={data} cx="50%" cy="50%" outerRadius="65%">
          ...
          <Tooltip /><Legend {...legendProps} />
          <Radar name={c('methods.a')} dataKey="a" stroke={C[0]} fill={C[0]} fillOpacity={0.25} />
          <Radar name={c('methods.b')} dataKey="b" stroke={C[1]} fill={C[1]} fillOpacity={0.25} />
          <Radar name={c('methods.c')} dataKey="c" stroke={C[2]} fill={C[2]} fillOpacity={0.25} />
        </RadarChart>
      </ResponsiveContainer>
    );
  }
  ...
  if (chartId === 'boxplot' || chartId === 'violin') {
    const data = [
      ...['A','B','C','D','E'].map((letter, i) => ({ x: `${c('groupPrefix')} ${letter}`, min: [10,15,20,12,18][i], q1: [25,30,35,28,32][i], median: [45,50,55,48,52][i], q3: [65,70,75,68,72][i], max: [90,85,95,88,92][i] })),
    ];
    return (
      <ResponsiveContainer width={w} height={h}>
        <ComposedChart data={data}>
          ...
          <YAxis tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Bar dataKey="median" name={ch('median')} fill={C[0]} radius={[3, 3, 0, 0]} barSize={40} />
          <Line type="monotone" dataKey="max" name={ch('max')} stroke={C[1]} strokeWidth={2} dot={false} />
          <Line type="monotone" dataKey="min" name={ch('min')} stroke={C[2]} strokeWidth={2} dot={false} />
        </ComposedChart>
      </ResponsiveContainer>
    );
  }
  ...
  if (chartId === 'heatmap') {
    ...
    return (
      <svg width={w} height={h} style={{ maxWidth: '100%' }}>
        ...
        <text x={30} y={h - 15} textAnchor="middle" style={{ fontSize: xAxisFontSize, fill: '#555' }}>{ch('xAxis')}</text>
        <text x={15} y={h / 2} textAnchor="middle" transform={`rotate(-90, 15, ${h / 2})`} style={{ fontSize: yAxisFontSize, fill: '#555' }}>{ch('yAxis')}</text>
      </svg>
    );
  }
  ...
  if (chartId === 'bubble') {
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <ReScatterChart>
          <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
          <XAxis type="number" dataKey="x" tick={xTickProps} label={{ ...xLabelProps, value: ch('xAxis') }} />
          <YAxis type="number" dataKey="y" tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          {groups.map((g, gi) => (
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <ReScatterChart>
          ...
          <YAxis type="number" dataKey="y" tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Scatter name={ch('up')} data={data.filter(d => d.sig === 'up')} fill={C[0]} shape="circle" r={3} />
          <Scatter name={ch('down')} data={data.filter(d => d.sig === 'down')} fill={C[1]} shape="circle" r={3} />
          <Scatter name={ch('ns')} data={data.filter(d => d.sig === 'ns')} fill="#ccc" shape="circle" r={2} />
        </ReScatterChart>
      </ResponsiveContainer>
    );
  }
  ...
  if (chartId === 'kmeans') {
    ...
    const data = centers.flatMap((center, ci) =>
      Array.from({ length: 40 }, (_, i) => ({
        x: Math.round((center[0] + (seededRandom(i * 0.1 + ci * 100) - 0.5) * 2) * 100) / 100,
        y: Math.round((center[1] + (seededRandom(i * 0.1 + ci * 100 + 50) - 0.5) * 2) * 100) / 100,
        cluster: `${c('clusterPrefix')} ${ci + 1}`, fill: C[ci % C.length]
      }))
    );
    return (
      <ResponsiveContainer width={w} height={h}>
        <ReScatterChart>
          ...
          <Tooltip /><Legend {...legendProps} />
          {[0, 1, 2, 3].map(ci => (
            <Scatter key={ci} name={`${c('clusterPrefix')} ${ci + 1}`} 
              data={data.filter(d => d.cluster === `${c('clusterPrefix')} ${ci + 1}`)} 
  ...
  if (chartId === 'umap') {
    ...
    const data = centers.flatMap((center, ci) =>
      Array.from({ length: 35 }, (_, i) => ({
        x: Math.round((center[0] + (seededRandom(i * 0.1 + ci * 100) - 0.5) * 3) * 100) / 100,
        y: Math.round((center[1] + (seededRandom(i * 0.1 + ci * 100 + 50) - 0.5) * 3) * 100) / 100,
        cell: `${c('cellPrefix')} ${String.fromCharCode(65 + ci)}`, fill: C[ci % C.length]
      }))
    );
    return (
      <ResponsiveContainer width={w} height={h}>
        <ReScatterChart>
          ...
          <Tooltip /><Legend {...legendProps} />
          {[0, 1, 2, 3, 4, 5].map(ci => (
            <Scatter key={ci} name={`${c('cellPrefix')} ${String.fromCharCode(65 + ci)}`}
              data={data.filter(d => d.cell === `${c('cellPrefix')} ${String.fromCharCode(65 + ci)}`)}
  ...
  if (chartId === 'survival') {
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <LineChart data={data}>
          <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
          <XAxis dataKey="t" tick={xTickProps} label={{ ...xLabelProps, value: ch('xAxis') }} />
          <YAxis domain={[0, 1.05]} tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Line type="stepAfter" dataKey="a" name={c('treatments.a')} stroke={C[0]} strokeWidth={2} dot={{ r: 3 }} />
          <Line type="stepAfter" dataKey="b" name={c('treatments.b')} stroke={C[1]} strokeWidth={2} dot={{ r: 3 }} />
          <Line type="stepAfter" dataKey="c" name={c('treatments.c')} stroke={C[2]} strokeWidth={2} dot={{ r: 3 }} />
        </LineChart>
      </ResponsiveContainer>
    );
  }
  ...
  if (chartId === 'roc') {
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <LineChart>
          <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
          <XAxis type="number" dataKey="x" domain={[0, 1]} tick={xTickProps} label={{ ...xLabelProps, value: ch('xAxis') }} />
          <YAxis type="number" dataKey="y" domain={[0, 1.05]} tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Line data={diag} dataKey="y" name={ch('random')} stroke="#999" strokeDasharray="5 5" strokeWidth={1} dot={false} />
          <Line data={dataA} dataKey="y" name={ch('modelA')} stroke={C[0]} strokeWidth={2} dot={false} />
          <Line data={dataB} dataKey="y" name={ch('modelB')} stroke={C[1]} strokeWidth={2} dot={false} />
          <Line data={dataC} dataKey="y" name={ch('modelC')} stroke={C[2]} strokeWidth={2} dot={false} />
        </LineChart>
      </ResponsiveContainer>
    );
  }
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <BarChart data={data}>
          ...
          <YAxis tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Bar dataKey="val" name={ch('change')} fill={C[0]} radius={[2, 2, 0, 0]}>{data.map((d, i) => <Cell key={i} fill={d.color} />)}</Bar>
        </BarChart>
      </ResponsiveContainer>
    );
  }
  ...
  if (chartId === 'manhattan') {
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <ReScatterChart>
          <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
          <XAxis type="number" dataKey="x" domain={[0, maxX]} tick={xTickProps} label={{ ...xLabelProps, value: ch('xAxis') }} />
          <YAxis type="number" dataKey="y" tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Scatter name={ch('snps')} data={chromData.filter(d => !d.sig)} fill="#bbb" shape="circle" r={2} />
          <Scatter name={ch('significant')} data={chromData.filter(d => d.sig)} fill={C[0]} shape="circle" r={3} />
        </ReScatterChart>
      </ResponsiveContainer>
    );
  }
  ...
  if (chartId === 'qqplot') {
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <ReScatterChart>
          <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
          <XAxis type="number" dataKey="x" domain={[0, maxVal]} tick={xTickProps} label={{ ...xLabelProps, value: ch('xAxis') }} />
          <YAxis type="number" dataKey="y" domain={[0, maxVal]} tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Scatter name={ch('pvalues')} data={data} fill={C[0]} shape="circle" r={3} />
  ...
  if (chartId === 'dendrogram') {
    const labels = Array.from({ length: 12 }, (_, i) => `${c('samplePrefix')} ${i + 1}`);
  ...
  if (chartId === 'sunburst') {
    ...
    const innerLabels = cha('inner');
    const inner = innerLabels.map((label, i) => ({ label, value: [0.35, 0.20, 0.15, 0.12, 0.10, 0.08][i], color: C[i] }));
    // Outer ring: subtypes
    const outerLabels = cha('outer') as unknown as string[][];
    ...
    return (
      <svg width={w} height={h} style={{ maxWidth: '100%' }}>
        <text x={cx} y={20} textAnchor="middle" style={{ fontSize: titleFontSize, fontWeight: 'bold', fill: THEME_COLORS.primary }}>{ch('title')}</text>
        {arcs.map((a, i) => (
          <g key={i}>
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <ComposedChart data={data}>
          ...
          <YAxis tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Bar dataKey="val" name={ch('expression')} fill={C[0]} radius={[3, 3, 0, 0]} barSize={40} />
          <Line type="monotone" dataKey="err" name={ch('error')} stroke={C[1]} strokeWidth={2} dot={{ r: 4 }} />
        </ComposedChart>
      </ResponsiveContainer>
    );
  }
  ...
  // ====== ???(????) ======
  if (chartId === 'sankey') {
    const stages = cha('stages');
    const targets = cha('targets');
  ...
  if (chartId === 'kegg-pathway') {
    ...
    return (
      <svg width={w} height={h} style={{ maxWidth: '100%' }}>
        ...
        <text x={w - 120} y={h * 0.15} textAnchor="start" style={{ fontSize: tickFontSize, fill: '#e74c3c', fontWeight: 'bold' }}>{ch('up')}: {upGenes.join(', ')}</text>
        <text x={w - 120} y={h * 0.15 + 18} textAnchor="start" style={{ fontSize: tickFontSize, fill: '#3498db', fontWeight: 'bold' }}>{ch('down')}: {downGenes.join(', ')}</text>
        {/* Legend */}
        <rect x={30} y={h - 45} width={14} height={14} fill={C[0]} opacity={0.8} rx={2} />
        <text x={50} y={h - 33} style={{ fontSize: tickFontSize, fill: '#555' }}>{ch('glycolysis')}</text>
        <rect x={140} y={h - 45} width={14} height={14} fill={C[6 % C.length]} opacity={0.8} rx={2} />
        <text x={160} y={h - 33} style={{ fontSize: tickFontSize, fill: '#555' }}>{ch('tca')}</text>
        <line x1={260} y1={h - 38} x2={280} y2={h - 38} stroke="#e74c3c" strokeWidth={2.5} />
        <text x={285} y={h - 33} style={{ fontSize: tickFontSize, fill: '#555' }}>{ch('upRegulated')}</text>
        <line x1={390} y1={h - 38} x2={410} y2={h - 38} stroke="#3498db" strokeWidth={2.5} />
        <text x={415} y={h - 33} style={{ fontSize: tickFontSize, fill: '#555' }}>{ch('downRegulated')}</text>
      </svg>
    );
  }
  ...
  if (chartId === 'gsea') {
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <ComposedChart>
          <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
          <XAxis type="number" dataKey="x" domain={[0, gseaN]} tick={xTickProps} label={{ ...xLabelProps, value: ch('xAxis') }} />
          <YAxis type="number" dataKey="y" domain={[-0.35, 0.7]} tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Area data={data} dataKey="y" name={ch('enrichmentScore')} fill={C[0]} fillOpacity={0.5} stroke={C[0]} strokeWidth={2} />
          <Scatter data={hits} dataKey="y" name={ch('leadingEdge')} fill={C[1]} shape="triangle" r={3} />
        </ComposedChart>
      </ResponsiveContainer>
    );
  }
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <ReScatterChart>
          ...
          <Tooltip /><Legend {...legendProps} />
          <Scatter name={ch('goTerms')} data={data} fill={C[0]}
            shape={(props: unknown) => {
  ...
  if (chartId === 'cnv') {
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <ComposedChart>
          <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
          <XAxis type="number" dataKey="x" domain={[0, 100]} tick={xTickProps} label={{ ...xLabelProps, value: ch('xAxis') }} />
          <YAxis type="number" domain={[-2, 2]} tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Scatter data={segs} dataKey="y" name={ch('segments')} fill={C[0]} shape="circle" r={2} />
  ...
  // ====== WGCNA??-????? ======
  if (chartId === 'wgcna') {
    const traits = cha('traits');
    const modules = cha('modules');
  ...
  if (chartId === 'fusion') {
    return (
      <svg width={w} height={h} style={{ maxWidth: '100%' }}>
        ...
        <text x={125} y={105} textAnchor="middle" style={{ fontSize: titleFontSize, fill: 'white', fontWeight: 'bold' }}>{ch('bcr')}</text>
        <rect x={350} y={80} width={150} height={40} fill={C[1]} opacity={0.8} rx={8} />
        <text x={425} y={105} textAnchor="middle" style={{ fontSize: titleFontSize, fill: 'white', fontWeight: 'bold' }}>{ch('abl1')}</text>
        ...
        <text x={300} y={185} textAnchor="middle" style={{ fontSize: titleFontSize, fill: 'white', fontWeight: 'bold' }}>{ch('fusion')}</text>
        <text x={125} y={60} textAnchor="middle" style={{ fontSize: tickFontSize, fill: C[0], fontWeight: 'bold' }}>{ch('chr22')}</text>
        <text x={425} y={60} textAnchor="middle" style={{ fontSize: tickFontSize, fill: C[1], fontWeight: 'bold' }}>{ch('chr9')}</text>
        <text x={300} y={230} textAnchor="middle" style={{ fontSize: xAxisFontSize, fill: '#555' }}>{ch('transcript')}</text>
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <BarChart data={data}>
          <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
          <XAxis dataKey="x" tick={xTickProps} label={{ ...xLabelProps, value: ch('xAxis') }} />
          <YAxis domain={[0, 100]} tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Bar dataKey="normal" name={ch('normal')} fill={C[0]} radius={[2, 2, 0, 0]} />
          <Bar dataKey="tumor" name={ch('tumor')} fill={C[1]} radius={[2, 2, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    );
  }
    ...
    return (
      <ResponsiveContainer width={w} height={h}>
        <AreaChart data={data}>
          ...
          <YAxis tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
          <Tooltip /><Legend {...legendProps} />
          <Area type="monotone" dataKey="s1" name={ch('sampleA')} fill={C[0]} fillOpacity={0.5} stroke={C[0]} />
          <Area type="monotone" dataKey="s2" name={ch('sampleB')} fill={C[1]} fillOpacity={0.5} stroke={C[1]} />
        </AreaChart>
      </ResponsiveContainer>
    );
  }
  ...
  return (
    <ResponsiveContainer width={w} height={h}>
      <LineChart data={DEMO_LINE_DATA}>
        <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
        <XAxis dataKey="name" tick={xTickProps} label={{ ...xLabelProps, value: ch('xAxis') }} />
        <YAxis tick={yTickProps} label={{ ...yLabelProps, value: ch('yAxis') }} />
        <Tooltip /><Legend {...legendProps} />
        <Line type="monotone" dataKey="geneA" name={c('genes.a')} stroke={C[0]} strokeWidth={2} dot={false} />
        <Line type="monotone" dataKey="geneB" name={c('genes.b')} stroke={C[1]} strokeWidth={2} dot={false} />
        <Line type="monotone" dataKey="geneC" name={c('genes.c')} stroke={C[2]} strokeWidth={2} dot={false} />
      </LineChart>
    </ResponsiveContainer>
  );
}
          ...
          <div className="lg:col-span-7 space-y-5">
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-primary transition-colors">
                <Icon name="customDraw" size={32} className="text-gray-400 mx-auto mb-2" />
              ...
              <div className="mb-5">
                ...
                <div className="space-y-2">
                  {COLOR_PALETTES.map((p, i) => (
                    <button key={p.nameKey} onClick={() => setPaletteIdx(i)}
                      ...
                      <span className={`text-xs font-medium ${paletteIdx === i ? 'text-primary' : 'text-gray-600'}`}>{t(p.nameKey)}</span>
                      {paletteIdx === i && <Icon name="check" size={12} className="text-primary ml-auto" />}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="p-3 bg-gray-50 rounded-lg text-xs space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-medium text-gray-700 ml-1">{t(palette.nameKey)}</span>
                  </div>
                </div>
                <div className="flex items-center justify-between">
